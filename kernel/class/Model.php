 ...
 */
	abstract class Model{
        ...
        // attributech sert à lister quels sont les attributs non-métiers : donc les attributs qui ne correspondent à rien dans la table
	...
	public function __construct($table, $pk, $autoincrement){
        ...
        $this->attributech = array('table', 'pk','autoincrement','attributech'); // pour ne pas les utilisers les attributs techniques des requêtes
	}
    ...
    **/
    protected function connexion(){
        ...
        try {
            $bdd = new PDO('pgsql:host='.$host.';dbname='.$dbname, $user, $mdp);
    ...
    public function create(){
        ...
        */
        if($this->autoincrement){ // si la clé primaire est en auto-incrément, 
            $this->attributech[]=$this->pk; // ALORS on l'ajoute dans le tableau des attributs techniques, comme ça , elle ne sera pas ajoutée dans la requête
        }
    ...
    */
    public function read() {
        ...
        $sql = "SELECT * FROM {$this->table} WHERE {$this->pk}={$this->{$this->pk}}";
    ...
    */
    public function update(){
        ...
        *                $this_>idutilisateur;
        * Voilà doinc ce qu'il va se passer : $this->{$this->pk}}
        *                                    = $this->idutilisateur;
        *                                     = 1  
        * Et c'est fini !
        */      
    ...
    */
	public function delete(){
        ...
        $req= "DELETE FROM {$this->table} WHERE {$this->pk} = {$this->{$this->pk}}";
        $sql= $bdd->prepare($req);
